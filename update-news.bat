@echo off
cd /d c:\Users\jakef\OneDrive\Desktop\HK\hatekid
powershell -ExecutionPolicy Bypass -Command "& {$content = Get-Content 'news.html' -Raw; $replacement = @'
            <article>
    <header>
...
'@; $newContent = $content -replace '            <article>.*?        </article>', $replacement, [System.Text.RegularExpressions.RegexOptions]::Singleline; $newContent | Set-Content 'news.html' -Encoding UTF8; Write-Host 'File updated successfully'}"
echo Done
pause
